...
## Decisions
- 2026-06-22: **Overhaul plan** drafted (`OVERHAUL_PLAN.md`). Confirmed scope: (1) build **backend/schema first**, then Web App UI, then bot; (2) Web App = **full CRUD + investments/FIRE** views, responsive primary UI; (3) **native Google sign-in** for UI (`access: MYSELF`/restricted), n8n stops anonymous POST (OAuth-as-owner or Sheets API); (4) Telegram bot adds **transfers + multi-tx, queries/read-back, corrections/undo**. Architecture: one GAS **service layer** owns all writes/rules; UI + n8n both call it. Schema adds stable tx **ID** column + compute-on-write derived fields. Open items: balance source (derive vs maintained), how derived cols are populated today, static vs live FX, investments read-only vs editable, n8n auth path.
