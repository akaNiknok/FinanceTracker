...
## TODO / open questions
- **NEXT SESSION (Phase 1 — backend/schema foundation):** plan locked in `OVERHAUL_PLAN.md`. Order: (1) owner runs the migration — `Migration.gs` `setupMigration()` then `applyDerivationFormulas()`, per `MIGRATION.md` (adds `ID` col + UUID backfill; converts Month/Type/Segment/Currency/Amount(PHP) to ARRAYFORMULA; ExchangeRate stays static input). (2) Refactor `Code.gs`/`Get.gs`/`Read.gs` into a service layer (`Router/Auth/Transactions/Accounts/Budgets/Calendar/Ledger/Interest.gs`). (3) Build the authenticated JSON API (`getBootstrap`, `listTransactions`, `create/update/deleteTransaction`, `createTransfer`, `getAccounts/updateAccount`, `getDashboard`, `getInvestments`). (4) Replace `appendRow` with input-columns-only range writes; live-FX stamp w/ per-tx override; idempotency key for n8n. (5) Auth migration `access: MYSELF` + n8n OAuth-as-owner Bearer; redeploy SAME deploymentId. (6) `Tests.gs` runners. Then Phase 2 (responsive Web App) & Phase 3 (bot: transfers/multi-tx, queries, undo).
