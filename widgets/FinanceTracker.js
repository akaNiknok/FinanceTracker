 ...
 *   segments  medium — Left to spend: Essentials + Rewards, as on Summary (tap → Dashboard)
...
const C = {
  ...
  accent: '#0A84FF', pos: '#30D158', neg: '#FF453A', ess: '#64D2FF', rew: '#FF9F0A', gro: '#BF5AF2'
};
const SEG = { Essentials: C.ess, Rewards: C.rew, Growth: C.gro };
...
// The tile label row: sentence case, --dim, never all caps (DESIGN.md "Typography").
function header(w, title, right) {
  const h = w.addStack();
  h.centerAlignContent();
  text(h, title, 12, C.dim, 'semi');
  h.addSpacer();
  if (right) text(h, right, 11, C.dim);
  w.addSpacer(8);
}
// The transaction row's icon tile: a rounded square with a 16% tint, or --card-2.
function glyph(stack, ch, color) {
  const s = stack.addStack();
  s.size = new Size(26, 26); s.cornerRadius = 8;
  s.backgroundColor = color ? col(color, 0.16) : col(C.card2);
  s.centerAlignContent();
  text(s, ch, 12, color || C.text, 'semi');
}
// DESIGN.md meter: 6px on --track, a 2px gap between stacked fills. fills = [[fraction, hex]].
function meter(w, fills, W) {
  const H = 6, dc = new DrawContext();
  dc.size = new Size(W, H); dc.opaque = false; dc.respectScreenScale = true;
  const pill = (x, wd, c) => {
    const p = new Path(); p.addRoundedRect(new Rect(x, 0, wd, H), H / 2, H / 2);
    dc.addPath(p); dc.setFillColor(col(c)); dc.fillPath();
  };
  pill(0, W, C.track);
  let x = 0;
  fills.forEach(([f, c]) => {
    const wd = Math.min(W - x, W * Math.max(0, f));
    if (wd >= 1) { pill(x, wd, c); x += wd + 2; }
  });
...
function recent(w, d) {
  ...
  d.recent.forEach((t, i) => {
    if (i) w.addSpacer(7);
    // The SPA's txView: a negative Expense is a refund, money in. The tile carries the
    // title's first letter tinted by segment; income is --pos, a transfer --accent with ⇄.
    const xfer = t.Type === 'Transfer' || !!t.ToAccount;
    const dir = xfer ? 0 : (t.Type === 'Expense' ? -1 : t.Type === 'Income' ? 1 : 0) * (Number(t.Amount) < 0 ? -1 : 1);
    const amtColor = xfer ? C.accent : dir < 0 ? C.neg : dir > 0 ? C.pos : C.text;
    ...
    glyph(r, xfer ? '⇄' : (title.charAt(0).toUpperCase() || '•'), xfer ? C.accent : dir > 0 ? C.pos : SEG[t.Segment]);
    r.addSpacer(8);
    const v = r.addStack(); v.layoutVertically();
    text(v, title, 12, C.text, 'semi');
    const sub = v.addStack();
    text(sub, (dir < 0 ? '−' : dir > 0 ? '+' : '') + amt, 11, amtColor, 'semi');
...
function balances(w, d) {
  if (!d.accounts.length) {
    ...
    w.addSpacer(4);
    const t = text(w, 'Choose them in the app: Admin, then iPhone balance widget.', 11, C.dim);
    t.lineLimit = 3;
    w.addSpacer();
    return;
  }
...
function networth(w, d, W) {
  ...
  w.addSpacer(4);
  // The Summary tile's bridge chip: a tinted pill with ▲/▼ and the change.
  const up = delta >= 0, chip = w.addStack();
  chip.backgroundColor = col(up ? C.pos : C.neg, 0.16); chip.cornerRadius = 7; chip.setPadding(2, 7, 2, 7);
  text(chip, (up ? '▲ ' : '▼ ') + compact(Math.abs(delta)) + ' in ' + s.length + ' months', 11, up ? C.pos : C.neg, 'bold');
  w.addSpacer();
  const img = w.addImage(sparkline(s.map((x) => x.value), W, 40));
  img.imageSize = new Size(W, 40);
  w.addSpacer(3);
  const lab = w.addStack();
  text(lab, s[0].month.slice(5), 10, C.dim);
  lab.addSpacer();
  text(lab, s[s.length - 1].month.slice(5), 10, C.dim);
}

// The Summary's Left to spend tile: what is left of Essentials + Rewards, one stacked
// meter in the segment colours, then a row per segment.
function segments(w, d, W) {
  const er = d.essentialsRewards;
  if (!er || er.targetPhp == null) { text(w, 'No Essentials or Rewards budget yet.', 12, C.dim); return; }
  const left = er.remainingPhp, fig = w.addStack();
  fig.bottomAlignContent();
  text(fig, left < 0 ? money(-left) + ' over' : money(left), 24, left < 0 ? C.neg : C.text, 'bold').minimumScaleFactor = 0.6;
  fig.addSpacer(6);
  text(fig, 'of ' + money(er.targetPhp), 13, C.dim);
  w.addSpacer(6);
  const by = {};
  d.segments.forEach((b) => { by[b.segment] = b; });
  meter(w, er.segments.map((n) => [er.targetPhp ? ((by[n] || {}).actualPhp || 0) / er.targetPhp : 0, SEG[n] || C.dim]), W);
  w.addSpacer(8);
  er.segments.filter((n) => by[n]).forEach((n, i) => {
    const b = by[n], rem = b.remaining, fmt = (x) => money(x, b.currency);
    if (i) w.addSpacer(5);
    const r = w.addStack(); r.centerAlignContent();
    const dot = r.addStack();
    dot.size = new Size(8, 8); dot.cornerRadius = 4; dot.backgroundColor = col(SEG[n] || C.dim);
    r.addSpacer(8);
    text(r, n, 12, C.text, 'semi');
    r.addSpacer(6);
    text(r, fmt(b.actual) + ' of ' + (b.target == null ? '—' : fmt(b.target)), 11, C.dim);
    r.addSpacer();
    text(r, rem == null ? '—' : rem >= 0 ? fmt(rem) + ' left' : fmt(-rem) + ' over', 12, rem != null && rem < 0 ? C.neg : C.text, 'semi');
  });
  w.addSpacer();
}
...
const DRAW = {
  ...
  segments: { title: 'Left to spend', fn: segments, family: 'medium' }
};
