...
| `Index.html`·`Stylesheet.html`·`App.html` | The responsive SPA: HTML shell · mobile-first CSS (bottom-nav on phone, sidebar ≥860px) · vanilla-JS app. Calls the service layer **directly via `google.script.run`** (no fetch URL/token; executes as owner). Screens: Dashboard/Transactions/Accounts/Budgets/Investments/Tax. Form/filter pickers are a custom **fuzzy combobox** (`comboEl`/`fuzzyScore`), not `<select>`. Account `color` is surfaced everywhere an account appears — Accounts rows, Transactions rows (left accent + dot via `acctColor`/`dotHTML`), Investments rows, and account-picker dropdown dots. Dates **display** as "Month D, YYYY" (`fmtDate`) but are sent to the server as ISO `yyyy-MM-dd` **strings** — see the Date gotcha below. |
