...
**JSON API (`?action=`):** reads — `getBootstrap`, `listTransactions` (filters month/account/category/segment/search + `limit`/`offset`), `getAccounts`, `getDashboard`, `getInvestments`, `getBudgets` (optional `month=`), `getRecurring`/`getCalendar`/`getLedger`/`getCategories`. writes (POST) — `createTransaction`, `createTransfer`, `updateTransaction`, `deleteTransaction`, `updateAccount`, `bulkUpdateTransactions`, `bulkDeleteTransactions`. Every tx has a stable server-assigned **`ID`**; `createTransaction` is idempotent if the caller passes an existing `ID`. Every write handler starts with `su_lock_()` (script lock, auto-released at execution end) so concurrent writes can't race the append-row calc or shifting row indices; bulk updates write one `getRangeList().setValue()` per patched field, not per cell.
...
**Screens (vanilla JS, no framework — GAS has no bundler):** Dashboard (`getDashboard`: net worth/assets/liabilities, budget bars, balances-by-type, recent), Transactions (`listTransactions` filters+pagination; add/edit/delete + transfer via modals; FAB), Accounts (`getAccounts` grouped by Type; edit interest/limit/notes via `updateAccount` — editable cols only), Budgets (`getBudgets`+`getRecurring`: segment bars, `essentialsRewards`, recurring list), Investments (`getInvestments`: positions/weights, **read-only**), Tax (`getLedger`: raw BIR table). **Responsive:** mobile-first; bottom-nav on phone, left sidebar ≥860px. A month picker (`yyyy-MMM`) drives period-aware screens. After any write the SPA re-hydrates `getBootstrap` (balances/budgets shift).
