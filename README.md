...
| Tests | 103 tests operate offline with `npm test`, and 68 of them use a real SQLite database |
...
| --- | --- |
| The bot sends no message at all. | The webhook. Send a new test message, then read `getWebhookInfo`. Do not set the webhook again first, because that action erases the last error. Then examine the secret `SECRET_TOKEN`. It must be the same as the script property `TELEGRAM_SECRET_TOKEN`. The bot reports each fault that it survives. Thus full silence means that the message did not arrive at the Worker. |
